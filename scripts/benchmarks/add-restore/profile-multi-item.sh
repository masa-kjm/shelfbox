...
# Manual P3 profiler for the 100 x 4 KiB explicit-path add and restore workflow.
...
Options:
  --samples <n>                 Add/restore samples (default: 10).
  --durability <require|best-effort>
                                Mutation durability (default: require).
  --help                        Show this message.

The runner builds and executes the ignored P3 profile test.
The output directory must not exist and must be located below this checkout's tmp/.
EOF
}

fail() {
    printf 'P3 profile failed: %s\n' "$*" >&2
    exit 1
}
...

{
    ...
    printf 'samples=%s\n' "$samples"
    printf 'file_count=100\n'
    printf 'file_size_bytes=4096\n'
    printf 'strategy=symlink\n'
    printf 'durability=%s\n' "$durability"
    printf 'timing=exclusive phase time; nested profiled work is charged to its own phase\n'
    printf 'profile_scope=shared ports and exclude session across 100 explicit file paths; context construction excluded\n'
    printf 'platform='
    uname -srm
} >"$metadata"

cd "$repo_root"
SHELFBOX_P3_PROFILE_OUTPUT="$raw_csv" \
SHELFBOX_P3_PROFILE_SAMPLES="$samples" \
SHELFBOX_P3_PROFILE_DURABILITY="$durability" \
    cargo test --release --locked -p shelfbox-core p3_multi_item_profile -- --ignored --nocapture \
...
by_operation_phase = defaultdict(list)
totals = {}
...
with open(raw_path, newline="", encoding="utf-8") as raw_file:
    for row in csv.DictReader(raw_file):
        ...
        by_operation_phase[(operation, phase)].append(value)
        key = (sample, operation)
        if phase == "total":
            totals[key] = value["elapsed_ms"]
        else:
            phase_sums[key] += value["elapsed_ms"]

if not totals:
    raise SystemExit("raw.csv has no total rows")

for (sample, operation), total in totals.items():
    by_operation_phase[(operation, "unattributed")].append(
        {"calls": 0, "bytes": 0, "elapsed_ms": max(0.0, total - phase_sums[(sample, operation)])}
    )
...
with open(summary_path, "w", newline="", encoding="utf-8") as summary_file:
    ...
    writer.writerow([
        "operation", "phase", "samples", "calls_min", "calls_max", "bytes_min", "bytes_max",
        "p50_ms", "p95_ms", "mean_ms",
    ])
    for operation, phase in sorted(by_operation_phase):
        values = by_operation_phase[(operation, phase)]
        ...
        writer.writerow([
            ...
            f"{nearest_rank(elapsed, 0.95):.3f}",
            f"{statistics.mean(elapsed):.3f}",
        ])
PY

printf 'P3 profile complete: %s\n' "$output_dir"
